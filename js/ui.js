                                        ...
                                        <div key={charge.id} className="p-4 hover:bg-card-soft transition-all duration-200 group relative">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
                                                ...
                                                <div className="bg-card-soft p-3 rounded-lg text-center border border-card-border hover:border-orange-400 transition-colors">
                                                    <div className="text-xs text-muted mb-1 flex items-center justify-center gap-1">⚡ kW medi</div>
                ...
                <div className="card">
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                        {filteredSuppliers.map(s => {
                            ...
                            return (
                                <div key={s.id} className={`card-soft p-3 hover:bg-card transition-colors flex justify-between items-center ${isDefault ? 'border-2 border-amber-500/50 bg-amber-500/10' : s.is_favorite ? 'border border-yellow-500/30 bg-yellow-500/5' : ''}`}>
                                    <div className="flex items-center gap-3">
                                        {/* Pulsante Medaglia - Fornitore di default */}
                                        <button
                                            onClick={() => onSetDefaultSupplier && onSetDefaultSupplier(s.id)}
                                            className={`text-xl transition-all ${isDefault ? 'opacity-100 scale-110' : 'opacity-30 grayscale hover:opacity-70 hover:grayscale-0'}`}
                                            title={isDefault ? "Fornitore di default (clicca per rimuovere)" : "Imposta come fornitore di default"}
                                        >
                                            🏅
                                        </button>
                                        <div>
