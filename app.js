...
function EVCostTracker() {
    ...
    function handleEditVehicleClick(vehicle) {
        // Mappiamo i dati corretti assicurandoci di passare l'ID
        setEditingVehicle({
            id: vehicle.id,
            name: vehicle.name,
            brand: vehicle.brand,
            // Importante: mappiamo capacity_kwh (dal DB) a capacity (per il form)
    ...
    async function handleSaveEditedVehicle() {
        ...
        if (ok) {
            ...
            // Importante: Ricarichiamo i dati dal server per vedere le modifiche
            await loadData(); 
